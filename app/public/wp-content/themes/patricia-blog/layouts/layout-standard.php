	...
	<article <?php post_class("post {$sticky_class}"); ?>>
		...
		<div class="entry-content">
			...
			<?php if( strpos( $post->post_content, '<!--more-->' ) ) : ?>
				...
				<span class="readmore">
					<a href="<?php the_permalink(); ?>" class="btn link-more"><?php esc_html_e( 'Read more', 'patricia-blog' ); ?><span class="dslc-icon fa fa-arrow-right"></span></a>
				</span>
			...
			<?php else : ?>
				...
				<div class="entry-summary">
					<?php
						the_content();
						wp_link_pages( array(
							'before' => '<div class="page-links">' . __( 'Pages:', 'patricia-blog' ),
							'after'  => '</div>',
						) );
					?>
				</div>
